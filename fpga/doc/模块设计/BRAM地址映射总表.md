...
| `SPECTRUM_BRAM` | 已规划 | 待 BD 分配 | 待 BD 分配 | 4096 点实 FFT 的正频率功率谱 |
...
- `Curve_mini` 曾把测试用频谱 BRAM 放在 `0x4300_0000`，该地址只能作为历史
  参考。新频域 BD 未完成前，不将它写成当前工程的正式地址。
...
当前功率数据暂定 40 位，而 AXI BRAM Controller 的 PS 常用端口为 32 位。
因此在以下事项确定之前，不定义 PS 绝对地址和 PS 打包格式：

1. 频谱 BRAM 是否直接开放给 PS；
2. 40 位功率值按 64 位对齐、拆成两个 32 位字，还是缩放成 32/16 位；
3. HMI 最终绘图需要 800 点、1024 点还是完整 2048 点；
4. 是否另设一个专供 PS/HMI 使用的压缩频谱显示 BRAM；
5. 最终 Block Memory Generator 的数据宽度、深度和端口配置。

完成频域 RTL 和 BD 后，必须在本节增加：
...
| W7 | `0x001C` | `energy_shift` | 当前规划固定为 8 |
