...
# The vivado2022 conda environment supplies VIVADO_HOME.  Set the variables
# normally prepared by Xilinx's batch loader explicitly so this script also
# works from a non-interactive PowerShell process.
...
$env:RDI_JAVACEFROOT = Join-Path $vivadoRoot 'tps\win64\java-cef-95.0.4638.69'
...
$sources = @(
    '..\src\testmodule\measurement_pipeline_top.sv',
    '..\src\hdl\adc_capture.sv',
    '..\src\hdl\fifo_wrap.v',
    '..\src\hdl\adc_write_controller.sv',
    '..\src\hdl\af_cdc.sv',
    '..\src\hdl\fifo_ctrl.sv',
    '..\src\hdl\peak_to_peak_detector.sv',
    '..\src\hdl\mean_square_calculator.sv',
    '..\src\hdl\measurement_bram_writer.sv',
    '..\src\sim\measurement_pipeline_top_tb.sv'
) | ForEach-Object { [System.IO.Path]::GetFullPath((Join-Path $currentWork $_)) }
$sources += (Join-Path $vivadoRoot 'data\verilog\src\glbl.v')

$resultDir = [System.IO.Path]::GetFullPath((Join-Path $currentWork '..\sim_results'))
New-Item -ItemType Directory -Force -Path $resultDir | Out-Null

& $xvlog --sv @sources
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& $xelab measurement_pipeline_top_tb glbl -L unisims_ver -s measurement_pipeline_top_tb_sim
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& $xsim measurement_pipeline_top_tb_sim -runall
exit $LASTEXITCODE
