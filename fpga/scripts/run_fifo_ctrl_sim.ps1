...
$rtl = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\src\hdl\fifo_ctrl.sv'))
$tb = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\src\sim\fifo_ctrl_tb.sv'))
...
& $xelab fifo_ctrl_tb -s fifo_ctrl_tb_sim
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& $xsim fifo_ctrl_tb_sim -runall
exit $LASTEXITCODE
