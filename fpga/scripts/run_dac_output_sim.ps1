...
$rtl = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\src\hdl\dac_output.sv'))
$tb = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\src\sim\dac_output_tb.sv'))
...
& $xelab dac_output_tb -s dac_output_tb_sim
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& $xsim dac_output_tb_sim -runall
exit $LASTEXITCODE
