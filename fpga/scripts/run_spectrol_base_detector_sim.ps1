...
$sources = @(
    '..\src\hdl\spectrol.sv',
    '..\src\hdl\base_detector.sv',
    '..\src\sim\spectrol_base_detector_tb.sv'
) | ForEach-Object { [System.IO.Path]::GetFullPath((Join-Path $currentWork $_)) }

& $xvlog --sv @sources
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& $xelab spectrol_base_detector_tb -s spectrol_base_detector_tb_sim
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& $xsim spectrol_base_detector_tb_sim -runall
exit $LASTEXITCODE
