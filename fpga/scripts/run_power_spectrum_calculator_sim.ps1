...
$sources = @(
    '..\src\hdl\power_spectrum_calculator.sv',
    '..\src\sim\power_spectrum_calculator_tb.sv'
) | ForEach-Object { [System.IO.Path]::GetFullPath((Join-Path $currentWork $_)) }

& $xvlog --sv @sources
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& $xelab power_spectrum_calculator_tb -s power_spectrum_calculator_tb_sim
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& $xsim power_spectrum_calculator_tb_sim -runall
exit $LASTEXITCODE
